const videojuegos = [
	{ nombre: 'The Legend of Zelda: Breath of the Wild', precio: 6500, id: 1 },
	{ nombre: 'God of War: Ragnarok', precio: 7200, id: 2 },
	{ nombre: 'Red Dead Redemption 2', precio: 5800, id: 3 },
	{ nombre: 'Elden Ring', precio: 6900, id: 4 },
	{ nombre: 'Minecraft', precio: 3000, id: 5 },
	{ nombre: 'POKEMON', precio: 6000, id: 6 },
	{ nombre: 'Call of Duty: Modern Warfare II', precio: 7500, id: 7 },
	{ nombre: 'Grand Theft Auto V', precio: 4000, id: 8 },
	{ nombre: 'Super Mario Odyssey', precio: 5500, id: 9 },
	{ nombre: 'Hogwarts Legacy', precio: 7000, id: 10 },
]

class Producto {
	constructor(nombre, precio, id) {
		this.nombre = nombre
		this.precio = precio
		this.id = id
		this.cantidad = 0
	}
}

// const fabricaDeProductos = () => {
// 	const auxArray = []

// 	for (let i = 0; i < videojuegos.length; i++) {
// 		let objetoAux = new Producto(
// 			videojuegos[i].nombre,
// 			videojuegos[i].precio,
// 			videojuegos[i].id
// 		)

// 		auxArray.push(objetoAux)
// 	}

// 	return auxArray
// }

const fabricaDeProductos = () => {
	const auxArray = videojuegos.map((videojuego) => {
		return new Producto(videojuego.nombre, videojuego.precio, videojuego.id)
	})

	return auxArray
}

const arrayProductosVideoJuegos = fabricaDeProductos()

const CarritoInstance = new Carrito()

// const mostradoraDeProductos = () => {
// 	let mensaje = 'Los prodcutos que tenemos son estos:\n '

// 	for (let i = 0; i < arrayProductosVideoJuegos.length; i++) {
// 		mensaje += `\n ID: ${arrayProductosVideoJuegos[i].id} - Nombre: ${arrayProductosVideoJuegos[i].nombre} - $ ${arrayProductosVideoJuegos[i].precio}`
// 	}

// 	alert(mensaje)
// }

const mostradoraDeProductos = (arrayDeProductos) => {
	let mensaje = arrayDeProductos.reduce(
		(acc, el) =>
			(acc += `\n ID: ${el.id} - Nombre: ${el.nombre} - $ ${el.precio}`),
		'Los productos que tenemos son estos:\n '
	)

	alert(mensaje)
}

const mostradoraDeProductosForEach = (arrayDeProductos) => {
	let mensaje = 'Los productos que tenemos son estos:\n '

	arrayDeProductos.forEach((el) => {
		mensaje += `\n ID: ${el.id} - Nombre: ${el.nombre} - $ ${el.precio}`
	})

	alert(mensaje)
}

const filtroMayorQue = (precio) => {
	let arrayAux = arrayProductosVideoJuegos.filter(
		(videojuego) => videojuego.precio > precio
	)

	mostradoraDeProductosForEach(arrayAux)
}

const filtroMenorQue = (precio) => {
	let arrayAux = arrayProductosVideoJuegos.filter((videojuego) => {
		return videojuego.precio < precio
	})

	mostradoraDeProductos(arrayAux)
}

const filtroPorPalabra = (nombreABuscar) => {
	let arrayAux = arrayProductosVideoJuegos.filter((videojuego) => {
		return videojuego.nombre
			.replace(/\s+/g, '')
			.toLowerCase()
			.includes(nombreABuscar.replace(/\s+/g, '').toLowerCase())
	})

	mostradoraDeProductos(arrayAux)
}

function core() {
	let bandera = true
	while (bandera) {
		let opciones = Number(
			prompt(
				'Bienvenidos a Perrito con Chaucha Store:\n 1-Ver productos\n 2-Comprar a partir de id \n 3-Ver carrito \n 4-Quitar producto a partir de un id \n 5-Filtrar por precio mayor a \n 6-Filtrar por precio mayor a \n 7-Filtrar por palabra \n 8-Comprar'
			)
		)

		switch (opciones) {
			case 0:
				bandera = false
				break
			case 1:
				mostradoraDeProductos(arrayProductosVideoJuegos)
				break
			case 2:
				let id = Number(prompt('¿Que ID de producto quiere comprar?'))
				let cantidad = Number(prompt('¿Cuantos quiere comprar?'))
				CarritoInstance.agregarProducto(id, cantidad)
				break
			case 3:
				CarritoInstance.mostradorDeCarrito()
				break
			case 4:
				let idBorrar = Number(prompt('¿Que ID de producto quiere comprar?'))
				let ctdBorrar = Number(prompt('¿Cuantos quiere borrar?'))
				CarritoInstance.quitarProductoDelCarrito(idBorrar, ctdBorrar)
				break
			case 5:
				let mayorQue = Number(prompt('¿De cuanto es el filtro?'))
				filtroMayorQue(mayorQue)
				break
			case 6:
				let menorQue = Number(prompt('¿De cuanto es el filtro?'))
				filtroMenorQue(menorQue)
				break
			case 7:
				let texto = prompt('¿De juego estas buscando?')
				filtroPorPalabra(texto)
				break
			case 8:
				CarritoInstance.terminarCompra()
				break
			default:
				break
		}
	}
}

core()

// function findDeLaSalada(callBack, arrayUser) {
// 	let boleanoAux = false

// 	for (let i = 0; i < arrayUser.length; i++) {
// 		boleanoAux = callBack(arrayUser[i], i, arrayUser)

// 		if (boleanoAux == true) {
// 			return arrayUser[i]
// 		}
// 	}
// }

// let funcionBuscadora = (el, id, array) => {
// 	return el.id == 4
// }

// let videojuegoABuscar = findDeLaSalada(funcionBuscadora, videojuegos)

// console.log(videojuegoABuscar)
