...
function router(pathOp) {
	...
	switch (path) {
		...
		case '/':
			app.innerHTML =
				'<h1>Inicio</h1><p>Bienvenido a la página de inicio. Perrito con chaucha</p>'
		...
		case '/about':
			app.innerHTML =
				'<h1>Acerca de</h1><p>Esta es la página de información acerca de nosotros.</p>'
			localStorage.setItem('hash', path)
		...
		case '/contact':
			app.innerHTML =
				'<h1>Contacto</h1><p>Puedes contactarnos mediante el formulario de contacto.</p>'
			localStorage.setItem('hash', path)
			inicio.classList.remove('focus')
			about.classList.remove('focus')
